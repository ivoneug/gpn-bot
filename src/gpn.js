...
const FALLBACK_FUELS = {
  ...
  541: 'ДТ Опти', 100032: 'G-100', 100036: '100',
};
...
/**
 * Приводит названия топлива к одному виду — короткому, как в shortTitle: "92",
 * "95", "G-95". Длинные написания срезаем: title приходит как "Бензин АИ-92",
 * запасной справочник исторически хранил "АИ-92".
 * Брендированные (G-95) и дизельные (ДТл) названия остаются как есть.
 */
function normalizeFuelTitle(raw) {
  const s = String(raw ?? '').replace(/\s+/g, ' ').trim();
  return s.replace(/^Бензин\s+/iu, '').replace(/^АИ-/iu, '');
}
